...
long_get_max_str_digits_xoption(int *pmaxdigits)
{
    ...
    key = PyUnicode_FromString("int_max_str_digits");
    if (key == NULL) {
        return -1;
    }
    ...
    maxdigits = _PyLong_AsInt(valuelong);
    Py_DECREF(valuelong);
...
#define _STRINGIFY(VAL) #VAL
    ...
    res = long_get_max_str_digits_xoption(&maxdigits);
    if (res == 1) {
        valid = ((maxdigits == 0) || (maxdigits >= _PY_LONG_MAX_STR_DIGITS_THRESHOLD));
        if (!valid) {
            res = -1;
